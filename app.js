...
const toastElMsg = document.getElementById('toast-message');
...
let toastTimerVar = null;
let isFullscreen = false;

function showToastFn(msg, duration = 1800) {
  if (!toastElMsg) return;
  toastElMsg.textContent = msg;
  toastElMsg.classList.add('show');
  clearTimeout(toastTimerVar);
  toastTimerVar = setTimeout(() => { toastElMsg.classList.remove('show'); }, duration);
}
...
function toggleFullscreen() {
  if (!isFullscreen) {
    ...
    isFullscreen = true;
    showToastFn('Fullscreen mode enabled', 1500);
  } else {
    document.body.classList.remove('fullscreen-mode');
    fullscreenBtn.textContent = '⛶';
    fullscreenBtn.title = 'Fullscreen';
    isFullscreen = false;
    showToastFn('Fullscreen mode disabled', 1500);
  }
}
...
function switchTab(id) {
  ...
  urlInputEl.value = activeTab ? (activeTab.displayUrl || '') : '';
  renderTabs();
}
...
async function doNavigate(tab, destUrl) {
  ...
  if (activeTabId === tab.id) urlInputEl.value = destUrl;
  renderTabs();
  ...
  if (!window.proxyReady()) {
    showToastFn('Initializing proxy...', 2000);
    await window.initProxy();
  }

  if (!window.proxyReady()) {
    showToastFn('Proxy not available', 3000);
    tab.loadState = 'error';
    tab.title = 'Error';
    renderTabs();
    return;
  ...
  try {
    ...
    showToastFn('Proxy error: ' + (err?.message ?? String(err)), 3000);
    tab.loadState = 'error';
    tab.title = 'Failed';
    renderTabs();
    return;
  ...
  if (!proxied) {
    showToastFn('Proxy returned empty', 3000);
    return;
  }
  ...
  function onFrameLoad() {
    ...
    if (activeTabId === tab.id) urlInputEl.value = tab.displayUrl;
    renderTabs();
  }
...
function createTab(initialUrl) {
  ...
  let tabObj = {
    ...
    loadState: 'idle', retryCount: 0, loadTimer: null
  };
  tabs.push(tabObj);
...
function updateEngineUI() {
  ...
  urlInputEl.placeholder = ph;
  document.querySelectorAll('.nt-search').forEach(el => { el.placeholder = ph; });
}

urlInputEl.addEventListener('keydown', e => {
  if (e.key === 'Enter' && urlInputEl.value.trim()) {
    navigateTab(activeTabId, urlInputEl.value);
  }
});
